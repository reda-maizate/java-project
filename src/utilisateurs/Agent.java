		...
		try {
			...
			while ((ligne = lecteur.readLine()) != null) {
				...
				if (identifiantA.equals(identifiantB)) {
					...
					System.out.println("Le statut du mandat que vous avez selectionné est " + manActuel.getStatut() + " (" + descriptionStatutMandat(manActuel.getStatut()) + ")");
					System.out.println();
					...
					System.out.println("Le statut de ce mandat est désormais : " + nouvStatut + " (" + descriptionStatutMandat(nouvStatut) + ")");
		...
		try {
			...
			while ((ligne = lecteur.readLine()) != null) {
				...
				if (mandatIdA == mandatIdB) {
					ecriture.write(man.getMandatId()+","+man.getBienId()+","+man.getAgenceId()+","+man.getAgentId()+","+permisConstruireDebut+","+permisConstruireFin+","+man.getNumPermis()+","+man.getStatut()+sauterLigne);
					ecriture.flush();
				} else {
			...
			if (entree.renameTo(new File("poubelle"))) System.out.println("Success creation poubelle");
			else System.out.println("Failure creation poubelle");
			if (sortie.renameTo(new File("mandats.txt"))) System.out.println("Success modification mandats");
			else System.out.println("Failure modififcation mandatd");
			if (entree.delete()) System.out.println("success poubelle deleted");
			else System.out.println("Failure poubelle deleted");
		}
	}
